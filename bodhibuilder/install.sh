...
# 32-bit install:
function 32b_install () {
  # Feedback
  echo " Running installation for a 32-bit system."
...
# vars
sysarch=`archdetect | cut -d/ -f1`
syssubarch=`archdetect | cut -d/ -f2`


# Run it based on architechture
case ${sysarch} in
  amd64 )
    64b_install
  ;;
